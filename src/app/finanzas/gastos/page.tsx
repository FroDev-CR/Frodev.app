...
export default function GastosPage() {
  ...
  useEffect(() => {
    getTransactions().then((t) => {
      setTxs(t.filter((x) => x.type === "gasto"));
      setLoaded(true);
    });
  }, []);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <header className="flex items-center justify-between">
        <h1 className="text-2xl font-bold uppercase rot-l">Gastos</h1>
        <button
          onClick={() => setShowForm((v) => !v)}
          className="brut-btn bg-expense text-white px-4 flex items-center gap-2"
      ...
      <div className="brut-card brut-card--expense p-4">
        <div className="flex items-center gap-2 text-expense">
          ...
          <span className="brut-tag bg-expense text-white">
            Gastado este mes
          </span>
        </div>
      ...
      <section>
        ...
        {loaded && txs.length === 0 && (
          <p className="text-muted text-sm">
            Sin gastos aún. Toca NUEVO para registrar el primero.
          </p>
        )}
        <ul className="flex flex-col gap-3">
          {txs.map((t) => (
            <li
              key={t.id}
              className="brut-card brut-card--expense p-3 flex justify-between items-center gap-2"
            >
              ...
              <div className="flex items-center gap-3 shrink-0">
                <p className="font-bold tabular-nums text-expense">
                  −{money(t.amount)}
                </p>
                <button
